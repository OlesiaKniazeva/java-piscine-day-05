...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        String sql = "UPDATE messages SET author_id = ?, room_id = ?, message = ?, date = ? WHERE id = ?;";
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            throw new NotSavedSubEntityException("Couldn't update message");
        }
    }
    ...
    public Optional<Message> findById(Long id) {
        ...
        try (Statement statement = connection.createStatement()){
            ...
            try (ResultSet resultSet = statement.executeQuery("SELECT * FROM rooms WHERE id=" + room_id)){
                ...
                if (time == null) {
                    return Optional.of(new Message(id, user, chatroom, text, null));
                }
